...
@WebServlet("/FacultyPasswordChange")
public class FacultyPasswordChange extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ResultSet resultset = statement.executeQuery("select password from faculty where password='" + cpassword
			...
			if (password.equals(confpass)) {
				if (pass.equals(cpassword)) {
					int i = statement.executeUpdate("update faculty set password='" + password + "' where uname='"
							+ session.getAttribute("uname") + "' ");
					String message = "Password change successfully.";
					session.setAttribute("password-change-success", message);
					response.sendRedirect("change_faculty_password.jsp");
					statement.close();
					con.close();
				} else {
