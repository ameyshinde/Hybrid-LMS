...
@WebServlet("/saveAptitude")
public class saveAptitude extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (time == null) {
		    ...
		    if (time <= 0) {
		      response.sendRedirect("apti_submit_result.jsp");
		      return;
		    }
		...
		try {
			...
			String query = "insert into apti_answers values(?,?,?,?)";
			String e = "";
			...
			if (sno == list.size()) {
				request.getRequestDispatcher("apti_submit_result.jsp").forward(request, response);
			}
			connection.close();
			request.getRequestDispatcher("aptianswersubmit.jsp").forward(request, response);
